...
# ── Process Pre-recorded Video (Optimal) ──────────────────────────────────

def process_prerecorded_video_streaming(video_path, holistic, normalizer, model, class_names, scaler, stframe, status_placeholder=None):
    """Process pre-recorded video with real-time streaming display."""
    ...
    while cap.isOpened():
        ...
        # Collect keypoints for prediction
        if results.left_hand_landmarks or results.right_hand_landmarks:
            keypoints, ref = extract_keypoints(results, frame.shape, normalizer)
            sequence_buffer.append(keypoints)
            cv2.circle(frame_display, (int(ref[0]), int(ref[1])), 6, (0, 0, 255), -1)

        # Display frame in real-time
        stframe.image(frame_display, channels="BGR", width=640)
        time.sleep(delay)

    cap.release()

    # After all frames processed, make prediction
    if len(sequence_buffer) >= SEQUENCE_LENGTH:
        if status_placeholder:
            status_placeholder.info("🔄 Memproses prediksi...")
        
        # Sample evenly distributed frames
        indices = np.linspace(0, len(sequence_buffer) - 1, SEQUENCE_LENGTH).astype(int)
        sampled_sequence = [sequence_buffer[i] for i in indices]

        # Predict
        seq_arr = np.array(sampled_sequence)
...
def main():
    ...
    with tab2:
        ...
        # Get available words from streamlit_videos directory
        ...
        if os.path.exists(video_dir):
            for video_file in os.listdir(video_dir):
                if video_file.endswith('.mp4'):
                    # Remove .mp4 extension and replace underscore with space
                    word = video_file.replace('.mp4', '').replace('_', ' ')
        ...
        if available_words:
            ...
            if selected_words:
                ...
                if process_sentence:
                    ...
                    normalizer = Normalizer()
                    sentence_results = []  # Store results for final summary
                    ...
                    with mp_holistic.Holistic(
                        ...
                        for word_idx, word in enumerate(selected_words):
                            st_status.info(f"🎬 Memproses video {word_idx + 1}/{len(selected_words)}: **{word}**")
                            st_progress.progress((word_idx) / len(selected_words))

                            # Get video file path (convert word back to filename format)
                            video_filename = word.replace(' ', '_') + '.mp4'
                            video_path = os.path.join(video_dir, video_filename)

                            if os.path.exists(video_path):
                                # Process video with real-time streaming
                                pred_class, conf = process_prerecorded_video_streaming(
                                    video_path, holistic, normalizer, model, class_names, scaler,
                                    stframe, st_status
                                )

                                if pred_class:
                                    # Generate TTS based on PREDICTION result, not filename
                                    st_status.success(f"✅ Hasil: **{pred_class}** ({conf:.1%})")
                                    sentence_results.append((word, pred_class, conf))
                                    
                                    # Generate and play audio for the predicted word
                                    audio_path = text_to_speech(pred_class)
                                    ...
                                    st.warning(f"⚠️ Video terlalu pendek atau tidak ada gesture untuk: {word}")
                                    sentence_results.append((word, "Tidak Dikenali", 0.0))

                            else:
                                st.warning(f"Video tidak ditemukan untuk kata: {word}")
                                sentence_results.append((word, "Video tidak ada", 0.0))
                        ...
                        for original, predicted, confidence in sentence_results:
                            if confidence > 0:
                                st.write(f"• **{original}** → {predicted} ({confidence:.1%})")
                            else:
                                st.write(f"• **{original}** → {predicted}")

            else:
                st.info("👆 Pilih kata-kata di atas untuk membuat kalimat")

        else:
            st.warning(f"Folder {video_dir} tidak ditemukan atau kosong. Silakan jalankan organize_videos_for_streamlit.py terlebih dahulu.")
