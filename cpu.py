...
from concurrent.futures import ProcessPoolExecutor, as_completed  # <-- Ditambahkan untuk Parallel CPU
...
SEQUENCE_LENGTH = 30
MP_MIN_WIDTH = 640
SHOW_PREVIEW = False  # Direkomendasikan tetap False karena GUI OpenCV tidak stabil di multi-processing
...
def process_video(video_path, holistic, class_name):
    ...
    if len(hand_indices) >= 5:
        ...
        total_frames = len(keypoints_data)
        # Di-comment agar tidak merusak visual progress bar tqdm saat multiprocessing
...
# --- FUNGSI WORKER UNTUK MULTI-PROCESSING ---
def process_single_video(video_info):
    """Fungsi pembantu khusus untuk memproses satu video di dalam core CPU terpisah."""
    video_path, class_name, out_class_dir, vid = video_info
    try:
        # Inisialisasi MediaPipe secara independen di setiap proses/core
...
def main():
    ...
    # Mengumpulkan semua daftar video dari seluruh kelas ke dalam satu antrean (Task List)
    all_tasks = []
    for class_name in classes:
        ...
        for vid in videos:
            ...
            all_tasks.append((video_path, class_name, out_class_dir, vid))

    print(f"Total videos to process: {len(all_tasks)}")

    # Eksekusi Parallel CPU menggunakan ProcessPoolExecutor
    # max_workers=None akan otomatis menggunakan seluruh core CPU yang tersedia di mesin Anda
    with ProcessPoolExecutor(max_workers=None) as executor:
        # Mendaftarkan seluruh tugas video ke core-core CPU
        futures = {executor.submit(process_single_video, task): task for task in all_tasks}

        # Menampilkan single progress bar utama untuk memantau semua video yang sedang diproses paralel
        for _ in tqdm(as_completed(futures), total=len(futures), desc="Processing Videos in Parallel"):
            pass
